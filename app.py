...
def load_config_and_model():
    ...
    tiler = PanoTiler(
        tile_size=config["dataset"]["patch_size"],
        overlap=32,
        upscale=config["model"]["upscale"],
    )
...
if model is None:
    st.error(
        "사전 학습된 모델 체크포인트를 찾을 수 없습니다. `checkpoints/pano_swinir_epoch_100.pth` 파일이 업로드되어 있는지 확인해 주세요."
    )
else:
    ...
    uploaded_file = st.file_uploader(
        "파노라마 X-ray 이미지 업로드", type=["png", "jpg", "jpeg", "dcm", "dicom"]
    )
    ...
    if uploaded_file is not None:
        ...
        try:
            img_hr_orig = (
                preprocessor.load_dicom(tmp_file_path)[0]
                if tmp_file_path.lower().endswith((".dcm", ".dicom"))
                else cv2.imread(tmp_file_path, cv2.IMREAD_UNCHANGED)
            )
    ...
    process_mode = st.sidebar.radio(
        "처리 모드 선택",
        ["직접 화질 개선 (실전 모드)", "화질 저하 시뮬레이션 (데모 모드)"],
        index=0,
    )
    ...
    if "실전 모드" in process_mode:
        ...
        hybrid_mode = st.sidebar.radio(
            ...
            [
                "AI 초해상도 확대 (2x SR)",
                "원본 해상도 유지 (1x Denoising + 후처리 Sharpening)",
            ],
    ...
    st.sidebar.caption(
        "치근, 임플란트 등 경계선을 선명하게 만들고 싶을 때 수치를 높이세요."
    )
    ...
    if uploaded_file is not None and img_hr_orig is not None:
        try:
            ...
            if orig_w >= 1200:
                st.warning(
                    "💡 **고해상도 파노라마 영상 감지 (가로 1200px 이상)**\n이미 화질이 충분히 좋은 상태입니다. 무리한 SR(초해상도) 연산은 오히려 디테일을 뭉개거나 환각(Hallucination)을 유발할 수 있습니다. 좌측 사이드바에서 **[원본 해상도 유지]** 옵션이 자동 선택되었습니다. 수동으로 변경하여 진행하실 수 있습니다.",
                    icon="⚠️",
                )
            elif orig_w < 800:
                st.success(
                    f"✅ **저해상도 소형 영상 감지 (가로 {orig_w}px)**\n작은 이미지의 미세한 고주파 성분을 정확히 추론하기 위해, 모델의 수용 영역(Receptive Field)과 타일 크기를 해상도에 비례하여 동적으로 조절합니다.",
                    icon="🔎",
                )
                ...
                if st.button(btn_label, use_container_width=True):
                    with st.spinner("처리 중입니다..."):
                        ...
                        if model_input_w != orig_w:
                            ...
                            st.info(
                                f"💡 **Single-pass 도메인 매칭**: 모델 최적화를 위해 {orig_w}px ➡️ {model_input_w}px 로 선확장되었습니다. (Lanczos4)"
                            )
                        ...
                        st.info(
                            f"🔍 **데이터 스케일 무결성 검증**: Tensor Min = `{min_val:.4f}`, Max = `{max_val:.4f}` (정상 범위: 0~1)"
                        )
                        if min_val < -0.1 or max_val > 1.1:
                            st.error(
                                "⚠️ 데이터 스케일이 [0, 1] 범위를 크게 벗어났습니다. 모델의 Activation이 오작동하여 수채화 현상이 발생할 수 있습니다."
                            )
                        ...
                        current_tensor = tiler.process_large_image(
                        ...
                        st.success(
                            f"⏱️ **Single-pass 추론 완료** (소요 시간: `{inference_sec:.2f}`초, 구동 디바이스: `{device}`)"
                        )
                        ...
                        st.session_state.history = [
                            {"img": res_img, "scale": initial_upscale}
                        ]
            ...
            for idx, item in enumerate(st.session_state.history):
                ...
                st.image(
                    output_img,
                    caption=f"Resolution: {output_img.shape[1]}x{output_img.shape[0]} (Sharpening: {sharpen_amount})",
                    clamp=True,
                    ...
                    with c1:
                        if st.button(
                            ...
                            with st.spinner(f"x{next_scale} 단계 추론 중..."):
                                ...
                                pre_img = preprocessor.preprocess_pipeline(
                                    tmp_file_path
                                )
                                ...
                                if model_input_w != orig_w:
                                    ...
                                    st.info(
                                        f"💡 **Single-pass 도메인 매칭**: {orig_w}px ➡️ {model_input_w}px 로 선확장되었습니다. (Lanczos4)"
                                    )

                                img_tensor = (
                                    torch.from_numpy(pre_img).float().unsqueeze(0)
                                )

                                start_time = time.time()
                                output_tensor = tiler.process_large_image(
                                    model, img_tensor, device
                                )
                                end_time = time.time()
                                inference_sec = end_time - start_time
                                st.success(
                                    f"⏱️ **Single-pass 추론 완료 (x{next_scale})** (소요 시간: `{inference_sec:.2f}`초, 구동 디바이스: `{device}`)"
                                )
                                ...
                                st.session_state.history.append(
                                    {"img": new_res, "scale": next_scale}
                                )
                                st.rerun()

                    with c2:
                        out_img_uint8 = (np.clip(output_img, 0, 1) * 255).astype(
                            np.uint8
                        )
            ...
            if (
                ...
                and st.session_state.history[-1]["scale"] >= 16
            ):
