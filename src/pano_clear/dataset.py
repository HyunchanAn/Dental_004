...
class PanoDataset(Dataset):
    ...
    def __init__(
        self, root_dirs, patch_size=128, upscale=2, mode="train", noise_level=0.05
    ):
            ...
            if os.path.exists(dentex_train_path):
                ...
                for sub in os.listdir(dentex_train_path):
                    ...
                    # unlabelled xrays ???ы븿
                        ...
                        unlabelled_path = os.path.join(
                            dentex_train_path, "unlabelled", "xrays"
                        )
                        if os.path.exists(unlabelled_path):
                            self.image_paths.extend(
                                glob.glob(os.path.join(unlabelled_path, "*.png"))
                            )
        ...
        print(
            f"[{mode}] 紐⑤뱶: {len(self.image_paths)} 媛쒖쓽 ?대?吏瑜?濡쒕뱶?덉뒿?덈떎."
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train":
            ...
            img_hr = img_hr[
                y_start : y_start + self.patch_size, x_start : x_start + self.patch_size
            ]
...
if __name__ == "__main__":
    # ?곗씠?곗뀑 濡쒕뱶 ?뚯뒪??
    dataset = PanoDataset(
        root_dirs=["data/raw/Tufts Dental Database", "data/raw/DENTEX_data"],
        patch_size=128,
    )
    if len(dataset) > 0:
        sample = dataset[0]
