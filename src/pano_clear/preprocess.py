...
class PanoPreprocessor:
    ...
    def load_dicom(self, path: str) -> Tuple[np.ndarray, float, float]:
        ...
        assert bits_stored in [12, 16], (
            f"지원하지 않는 Bit Depth 입니다: {bits_stored}. 12-bit 또는 16-bit DICOM만 지원합니다."
        )
        ...
        if hasattr(ds, "WindowCenter") and hasattr(ds, "WindowWidth"):
            ...
            ww = ds.WindowWidth
            wc_list = (
                wc if hasattr(wc, "__iter__") and not isinstance(wc, str) else [wc]
            )
            ww_list = (
                ww if hasattr(ww, "__iter__") and not isinstance(ww, str) else [ww]
            )
    ...
    def apply_windowing(
        self, img: np.ndarray, window_center: float = None, window_width: float = None
    ) -> np.ndarray:
        """
    ...
    def preprocess_pipeline(self, path: str) -> np.ndarray:
        ...
        if path.lower().endswith((".dcm", ".dicom")):
            ...
            # 만약 RGB라면 그레이스케일로 변환
            if img.ndim == 3:
                img = cv2.cvtColor(img.astype(np.uint8), cv2.COLOR_BGR2GRAY).astype(
                    np.float32
                )
