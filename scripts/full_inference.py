...
def full_inference():
    ...
    test_img_path = os.path.join(
        config["dataset"]["root_dirs"][0], "Radiographs", "1.JPG"
    )
    ...
    img_lr_input = cv2.resize(
        img_hr_orig,
        (w // config["model"]["upscale"], h // config["model"]["upscale"]),
        interpolation=cv2.INTER_CUBIC,
    )

    # ?몄씠利?異붽?
    noise = np.random.normal(
        0, config["dataset"]["noise_level"], img_lr_input.shape
    ).astype(np.float32)
    ...
    full_result_path = os.path.join(
        config["path"]["results"], "full_panorama_result.png"
    )
    ...
    # 媛쒕퀎 ?대?吏 ???
    cv2.imwrite(
        os.path.join(config["path"]["results"], "result_output.png"),
