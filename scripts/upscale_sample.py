...
def upscale_sample_image(input_path, output_path):
    ...
    tiler = PanoTiler(
        tile_size=config["dataset"]["patch_size"], overlap=32, upscale=upscale
    )
    ...
    print(
        f"Original size: {img_lr.shape[1]}x{img_lr.shape[0]} -> Upscaled size: {output_img_uint8.shape[1]}x{output_img_uint8.shape[0]}"
    )
